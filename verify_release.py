...
MANIFEST = {
    ...
    "_test_scratch.py": ("c40f54ef0cbfa379ce81d638679ff584143cb878394000e3de038509c07eebfa", 796672, "v8.18.0 W1-A: + T-W1A-01..11 (Final Decision Status precedence / verdict-code classifier / marker+auto-downgrade / secondary reasons never override / UNGRADED result-only / no-MISTAKE-without-error / serialization / pill-never-blank / contradiction gate catches+passes / ONE-owner stamp); REV16 base preserved"),
    ...
    "gem_commentary_capsule.py": ("3f2e65ce420bee01bf2406b2d764dd115ffe8c83757b2ca4e2de2c8dc4578e3d", 16298, "v8.17 Epic A: commentary capsule layer (registers + evidence tiers + capsule builder + content lints L1-L13)"),
    ...
    "gem_report_draft/_html.py": ("cd52d638d49d4a53fc6f577fad81821f00a7f70eab7183b0af5e6dc2202341ae", 433305, "v8.18.0 W1-A: .final-status-pill CSS (mistake/conditional/cleared/ungraded, desktop+mobile) + the sticky topbar mirrors the canonical status pill + the inline-whitelist passes final-status-pill"),
    ...
    "gem_report_draft/sections_tournaments.py": ("dc2ecf9056832c1865673acc6d0584f1a44f807a997856fb22f9574470cc2df0", 43398, "v8.17.1 P4: Tournament Tables v3 -- 7 surfaces (grouped aggregates, distribution chart, filters/sticky, Finance&Finish, Performance, drivers rollup) unified into one Tournament Results block"),
    ...
    "gem_villain_teaching.py": ("11e9abe0feb661532180db2f7f877da851645cd0ddc0be30a2880ddd9c2dc486", 47468, "v8.16.0 villain: status-safe cards + grade gate + calibrated node-specific mixed/split caveat + ICM caution"),
...
CANARIES = [
    ...
    ("gem_report_draft/sections_tournaments.py", "_dthand(_RES_COLS[9]",
