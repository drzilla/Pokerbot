...
MANIFEST = {
    ...
    "_test_scratch.py": ("bbcfe8ad9c551a9f539be3350ed4dc2b6f1347b6fe0f9299b8e4a2a3a5ee998e", 660053, "v8.17.1 Iter-1 REV3: + T-I1F-01..26 decision-time bounty (prefix invariance, cover!=eligibility, exhaustive truth table), folded dead-money pot layers, and gate-catches-injected-bug tests"),
    ...
    "gem_analyst_worklist.py": ("f81bc8465fcf6813ae6ef0c69c46bfa0554bc457a6b1478a210953b443f274fc", 62104, "v8.17.1 Iter-1 REV3: bounty_context consumes the ONE canonical FUTURE-BLIND build_decision_bounty_context (eligibility + stack-cover are separate fields; one typed aggregate/reason)"),
    "gem_analyzer.py": ("acf377942b4c781deb6101d3d93056cdd45273ae511fc6963d267ca6fdd9fac2", 594866, "v8.17.1 Iter-1 REV3: also stamps the canonical FUTURE-BLIND decision_bounty_context onto the report-rendered hand (report consumes the SAME object the worklist grades from)"),
    ...
    "gem_decision_snapshot.py": ("dec4020975918de4572c5e432f3bb4d100916356fe706edd5a9987ac3983e3d6", 46919, "v8.17.1 Iter-1 REV3: + build_decision_bounty_context (future-blind decision-time owner), _classify_bounty (one typed aggregate/reason), _contest_pot_layers (folded dead money in the pot, reconciling layers); realized accessors no longer fall back to the stack-cover map"),
    ...
    "gem_report_draft/sections_xiv.py": ("22be0f8ca08a5a0b7a239cd95b66c6837aca611eea80f9a5b73bd8d8b836f7a3", 246515, "v8.17.1 Iter-1: bounty-math note distinguishes an uncontested jam (no confrontation) from a genuine data gap (canonical contesting_count)"),
...
CANARIES = [
    ...
    ("gem_decision_snapshot.py", "def _normalize_uncalled(",
     "v8.17.1 Iter-1 REV5 B3: uncalled excess is returned, not represented as a one-player side pot"),
