...
MANIFEST = {
    ...
    "_test_scratch.py": ("cf254338639e148b83b135d48218bdd7146c03659f4ab9f16baeb547eadca1d1", 693056, "v8.17.1 Iter-1 REV6: + T-LU-01..10 ledger-uncalled, T-B2-01..09 visible-decision routing + failure injection, T-B34-01..09 combined exact+potential applicability + certainty + auto-clear"),
    ...
    "gem_analyzer.py": ("06abde3dde00194ef0fc01d1f93aa1e4dcaff316c6debeae706997e45dd188e1", 596267, "v8.17.1 Iter-1 REV6: stamps the ledger-inferred reviewed_decision_ref onto every hand (visible decision routing independent of render path)"),
    ...
    "gem_coaching_cards.py": ("4c88bb7316339d6e2c7f86eb26d034c710a8499170937ea1cb77fddb4cc47c39", 48795, "v8.17.1 Iter-1: blocker uses board_at_decision (no future-board reads); multiway uses canonical contesting_count (not table size); pko_pressure pinned to preflop"),
    ...
    "gem_decision_snapshot.py": ("ff2d08536d8f3423185b884641192a977a51f290e008d480056cb131274dee58", 69612, "v8.17.1 Iter-1 REV6: _ledger_uncalled (ledger-derived uncalled returns), build_reviewed_decision_ref + infer_reviewed_action_index (canonical visible routing), exact_and_potential applicability + bounty_certainty"),
    ...
    "gem_report_draft/sections_xiv.py": ("8f27e67aa7c5fee52a68fe53b46dd09d53bccb8a6ace2d86a72cc0d0935e2ee2", 268579, "v8.17.1 Iter-1 REV6: XIV.A + XIV.B capsule/pot-odds route through the canonical reviewed-decision ref (visible Reviewed-decision line + data-decision-action-index); combined exact+potential bounty disclosure"),
