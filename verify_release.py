...
MANIFEST = {
    ...
    "_test_scratch.py": ("83136d3b27998efc68ea148c671e6e8cfc8b4e20ebff5a2ef7b74550e94639f0", 632643, "v8.17.1 Iter-1 REV3: + T-I1F-01..26 decision-time bounty (prefix invariance, cover!=eligibility, exhaustive truth table), folded dead-money pot layers, and gate-catches-injected-bug tests"),
    ...
    "gem_analyst_worklist.py": ("f2f31aaa1a8f2e67222355114c4a695050c6bb82d730778e4c2f6c7bb976e757", 59729, "v8.17.1 Iter-1 REV3: bounty_context consumes the ONE canonical FUTURE-BLIND build_decision_bounty_context (eligibility + stack-cover are separate fields; one typed aggregate/reason)"),
    "gem_analyzer.py": ("d6da3278cca96f2ae899564725373d1d35d32d40e84852e88ee9afd83e694a81", 593585, "v8.17.1 Iter-1 REV3: also stamps the canonical FUTURE-BLIND decision_bounty_context onto the report-rendered hand (report consumes the SAME object the worklist grades from)"),
    ...
    "gem_coaching_cards.py": ("573457fa0bd84a7b6b6d7cee7a61ecd842aee6939f327c3dbecc145fc28b33e8", 48910, "v8.17.1 Iter-1: blocker uses board_at_decision (no future-board reads); multiway uses canonical contesting_count (not table size); pko_pressure pinned to preflop"),
    ...
    "gem_decision_snapshot.py": ("9c4b50f3f8d4049c0515db0f788f131d225664331ca3f0d1063856617d73021f", 37853, "v8.17.1 Iter-1 REV3: + build_decision_bounty_context (future-blind decision-time owner), _classify_bounty (one typed aggregate/reason), _contest_pot_layers (folded dead money in the pot, reconciling layers); realized accessors no longer fall back to the stack-cover map"),
    ...
    "gem_report_draft/sections_xiv.py": ("99b79cdd60c3b08473adf33eb53e05f814b6b589b6cf9b83e4b3147e607aea90", 240066, "v8.17.1 Iter-1: bounty-math note distinguishes an uncontested jam (no confrontation) from a genuine data gap (canonical contesting_count)"),
...
CANARIES = [
    ...
    ("gem_analyzer.py", "h['decision_bounty_context'] = _ds_dbc(h)",
     "v8.17.1 Iter-1 REV3 B1: report-rendered hand carries the same canonical decision-time object"),
